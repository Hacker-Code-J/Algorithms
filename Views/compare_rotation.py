...
ax.plot(function1_smooth, color='dodgerblue', label='Juggling', alpha=0.8, linewidth=2)
ax.plot(function2_smooth, color='crimson', label='Block-Swap', alpha=0.8, linewidth=2)
ax.plot(function3_smooth, color='forestgreen', label='Reverse', alpha=0.8, linewidth=2)

# Add horizontal lines to indicate the average values
ax.hlines(average_function1, xmin=0, xmax=len(function1_smooth), colors='dodgerblue', linestyles='dotted', label=f'Avg Juggling: {average_function1:.2f} μs')
ax.hlines(average_function2, xmin=0, xmax=len(function2_smooth), colors='crimson', linestyles='dotted', label=f'Avg Block-Swap: {average_function2:.2f} μs')
ax.hlines(average_function3, xmin=0, xmax=len(function3_smooth), colors='forestgreen', linestyles='dotted', label=f'Avg Reverse: {average_function3:.2f} μs')

# Setting labels, title, and legend with a prettier font and style
ax.set_ylabel('Speed (μs)', fontsize=14, fontweight='bold')
